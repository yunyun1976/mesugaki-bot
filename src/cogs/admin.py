...
class Admin(BaseCog):
    async def _get_phrases_paginated(self, interaction: discord.Interaction, category: Category):
        if not await check_authorized(interaction):
            return

        phrases = await self.bot.phrase_repo.get_all(category)

        if not phrases:
            await send_response(interaction, MessageHandler.get('admin.phrases_empty'))
            return

        view = PaginationView(phrases, category.list_title)
        await view.update_buttons()
        embed = await view.get_page_content()
        await send_response(interaction, embed=embed, view=view)

    async def _remove_phrase_helper(self, interaction: discord.Interaction, category: Category, phrase: str):
        if not await check_authorized(interaction):
            return

        if await self.bot.phrase_repo.remove(category, phrase):
            await send_response(interaction, MessageHandler.get('admin.remove_success', phrase=phrase))
        else:
            await send_response(interaction, MessageHandler.get('admin.remove_failed', phrase=phrase), ephemeral=True)
    ...
    async def get_barizougon(self, interaction: discord.Interaction):
        await self._get_phrases_paginated(interaction, Category.BATOU)
    ...
    async def get_abikyoukan(self, interaction: discord.Interaction):
        await self._get_phrases_paginated(interaction, Category.WAKARASE)
    ...
    async def remove_batou(self, interaction: discord.Interaction, phrase: str):
        await self._remove_phrase_helper(interaction, Category.BATOU, phrase)
    ...
    async def remove_wakarase(self, interaction: discord.Interaction, phrase: str):
        await self._remove_phrase_helper(interaction, Category.WAKARASE, phrase)
    ...
    async def add_master(self, interaction: discord.Interaction, user: discord.User):
        ...
        if master_handler.add_master(user.id):
            await send_response(interaction, MessageHandler.get('admin.master_added', user_mention=user.mention), ephemeral=True)
        else:
            await send_response(interaction, MessageHandler.get('admin.master_already_exists', user_mention=user.mention), ephemeral=True)
    ...
    async def check_master(self, interaction: discord.Interaction):
        ...
        if not masters:
            await send_response(interaction, MessageHandler.get('admin.master_list_empty'), ephemeral=True)
            return
    ...
    async def remove_master(self, interaction: discord.Interaction, user: discord.User):
        ...
        if master_handler.remove_master(user.id):
            await send_response(interaction, MessageHandler.get('admin.master_removed', user_mention=user.mention), ephemeral=True)
        else:
            await send_response(interaction, MessageHandler.get('admin.master_remove_failed', user_mention=user.mention), ephemeral=True)
    ...
    async def set_channel(self, interaction: discord.Interaction):
        ...
        await send_response(interaction, MessageHandler.get('admin.channel_set'), ephemeral=True)
    ...
    async def sync(self, interaction: discord.Interaction):
        ...
        await send_response(interaction, MessageHandler.get('admin.sync_success', count=len(synced)), ephemeral=True)
